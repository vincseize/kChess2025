...
// ==============================================
// 4. LOGIQUE DYNAMIQUE POUR LES LABELS
// ==============================================

// Récupérer les paramètres de la partie
$playerColor = isset($_GET['color']) ? $_GET['color'] : 'white';
$gameMode = isset($_GET['mode']) ? $_GET['mode'] : 'human';
$botLevel = isset($_GET['level']) ? intval($_GET['level']) : 0;
$isBotGame = ($gameMode === 'bot' && $botLevel > 0);

// Si le joueur choisit noir, le plateau est retourné de base
$isBoardFlipped = ($playerColor === 'black');

// Déterminer qui contrôle chaque couleur
$whiteController = 'human'; // Par défaut
$blackController = 'human'; // Par défaut

if ($isBotGame) {
    if ($playerColor === 'white') {
        // Joueur = Blancs, Bot = Noirs
        $whiteController = 'human';
        $blackController = 'bot';
    } else {
        // Joueur = Noirs, Bot = Blancs
        $whiteController = 'bot';
        $blackController = 'human';
    }
}

// Déterminer qui est en haut/en bas
if ($isBoardFlipped) {
    // Plateau retourné : Blancs en haut, Noirs en bas
    $topColor = 'white';
    $bottomColor = 'black';
    $topController = $whiteController;
    $bottomController = $blackController;
} else {
    // Plateau normal : Noirs en haut, Blancs en bas
    $topColor = 'black';
    $bottomColor = 'white';
    $topController = $blackController;
    $bottomController = $whiteController;
}

// Fonction pour générer un label
function generatePlayerLabel($color, $controller, $translations, $isTop = false) {
    $colorText = ($color === 'white') 
        ? $translations['white_player'] 
        : $translations['black_player'];
    
    $controllerText = ($controller === 'bot')
        ? $translations['computer_player']
        : $translations['human_player'];
    
    $controllerIcon = ($controller === 'bot')
        ? 'bi-cpu'
        : 'bi-person';
    
    $bgClass = $isTop ? 'bg-dark text-white' : 'bg-white text-dark border border-dark';
    
    return <<<HTML
    <span class="badge {$bgClass} p-2">
        <i class="bi {$controllerIcon} me-1"></i> {$controllerText} {$colorText}
    </span>
HTML;
}

// Générer les labels
$topPlayerLabel = generatePlayerLabel($topColor, $topController, $translations, true);
$bottomPlayerLabel = generatePlayerLabel($bottomColor, $bottomController, $translations, false);
?>
    ...
    <div class="row g-3">
        ...
        <div class="col-xxl-8 col-xl-6 col-lg-6 col-md-8 col-12">
            <div class="chess-container bg-light rounded-3 p-2 h-100 position-relative">
                <!-- Label du joueur en haut -->
                <div class="position-absolute top-0 start-0 m-2">
                    <div id="topPlayerLabel">
                        <?php echo $topPlayerLabel; ?>
                    </div>
                </div>
                
                <!-- Label du joueur en bas -->
                <div class="position-absolute bottom-0 start-0 m-2">
                    <div id="bottomPlayerLabel">
                        <?php echo $bottomPlayerLabel; ?>
                    </div>
                </div>
                    ...
                    <div class="card-body p-2">
                        <div class="game-status text-center">
                            ...
                            <div class="small text-muted mb-2" id="currentPlayer">
                                ...
                                $startingPlayer = 'white'; // Par défaut, les Blancs commencent
                                $startingText = ($startingPlayer === 'white')
                                    ? $translations['traitAuBlancs']
                                    : $translations['traitAuxNoirs'];
                                echo $startingText;
                                ?>
                            </div>
