...
updateBotIndicator() {
    ...
    if (this.constructor.consoleLog) {
        console.log('🤖 [ChessGameUI] Mise à jour indicateur bot...');
        console.log('🤖 [ChessGameUI] Bot actif:', botStatus.active, 'Niveau:', botStatus.level, 'Couleur:', botStatus.color);
    }
    ...
    if (botStatus.active) {
        ...
        // 100% TRADUCTIONS JSON
        switch(botStatus.level) {
            case 1:
                botType = t.random_bot; // "Niveau 1 - Aléatoire" / "Level 1 - Random"
                botIcon = '🤖';
                break;
            case 2:
                botType = t.ccmo_bot; // "Niveau 2 - CCMO" / "Level 2 - CCMO"
                botIcon = '🧠';
                break;
            default:
                botType = t[`bot_level${botStatus.level}`] || t.unknown;
                botIcon = '🤖';
        }
        ...
        // Utiliser les traductions JSON pour le titre
        const colorText = botStatus.color === 'white' 
            ? (t.white || t.white_player)
            : (t.black || t.black_player);
        
        const playerType = t.computer_player;
        const playsText = t.plays;

        botIndicatorElement.innerHTML = `
            <span class="bot-indicator" title="${playerType} - ${playsText} ${colorText}">
                ${botIcon} ${botType}
            </span>
        `;
        ...
        if (isBotTurn) {
            ...
            currentPlayerElement.title = `${botType} ${t.bot_thinking}`;
            if (this.constructor.consoleLog) console.log('🤖 [ChessGameUI] C\'est le tour du bot');
        } else {
